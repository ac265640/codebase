...
# Use a writable directory in the Render environment for all data.
# This is the root cause of the server crashing.
...
# Init ChromaDB client in the writable directory
...
def clone_repo(repo_url: str = Form(...)):
    ...
    # Use the writable repos directory
...
def create_embeddings(repo_name: str = Form(...)):
    ...
    failed_files, collection = [], get_repo_collection(repo_name)
    batch_size = 96
    ...
    for path, content in read_files(repo_path):
        ...
        if len(docs_batch) >= batch_size:
            ...
            finally: docs_batch, ids_batch, metadatas_batch = [], [], []
    if docs_batch:
        try:
            res = requests.post("https://api.cohere.com/v1/embed", headers={"Authorization": f"Bearer {COHERE_API_KEY}"}, json={"texts": docs_batch, "model": "small", "truncate": "END"})
            res.raise_for_status()
